package com.revature.daoimpl;

import java.sql.CallableStatement;
import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.ResultSet;
import java.sql.SQLException;
import java.util.ArrayList;

import com.revature.beans.AppRequest;
import com.revature.beans.Holding;
import com.revature.dao.RequestsDao;
import com.revature.util.ConnFactory;

public class RequestsDaoImipl implements RequestsDao {
	public static ConnFactory cf = ConnFactory.getInstance();
	private Holding h = new Holding();
	private String month(String m) {
		int mx = Integer.parseInt(m);
		switch (mx) {
		case 1:
			return "JAN";
		case 2:
			return "FEB";
		case 3:
			return "MAR";
		case 4:
			return "APR";
		case 5:
			return "MAY";
		case 6:
			return "JUN";
		case 7:
			return "JUL";
		case 8:
			return "AUG";
		case 9:
			return "SEP";
		case 10:
			return "OCT";
		case 11:
			return "NOV";
		case 12:
			return "DEC";

		}
		return null;
	}

	private String formatDate(String d) {
		String[] split = d.split("-");
		StringBuffer sb = new StringBuffer();
		sb.append(split[2] + "-");
		sb.append(this.month(split[1]) + "-");
		sb.append(split[0]);
		return sb.toString();
	}

	@Override
	public void insertRequest(int userID, String name, String location, String date, String description, int type,
			int gradingScale, String passingGrade, String justification, double cost, double reimbursement,
			String links) {
		Connection conn = cf.getConnection();

		// format date
		String[] split = date.split("-");
		StringBuffer sb = new StringBuffer();
		sb.append(split[2] + "-");
		sb.append(this.month(split[1]) + "-");
		sb.append(split[0]);
		date = sb.toString();
		System.out.println("the date is this now " + date);
		String sql = "{ call insertRec(?,?,?,?,?,?,?,?,?,?,?,?)";
		CallableStatement call;
		try {

			call = conn.prepareCall(sql);
			call.setInt(1, userID);
			call.setString(2, name);
			call.setString(3, location);
			call.setString(4, date);
			// System.out.println("The date looks like this " + date);
			call.setString(5, description);
			call.setInt(6, type);
			call.setInt(7, gradingScale);
			call.setString(8, passingGrade);
			call.setString(9, justification);
			call.setDouble(10, cost);
			call.setDouble(11, reimbursement);
			call.setString(12, links);
			call.execute();

		} catch (SQLException e) {
			//
			e.printStackTrace();
		}

	}

	public int getUserId(int recid) {
		Connection conn = cf.getConnection();
		String sql = "select userid from request where requestid=?";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ps.setInt(1, recid);
			ResultSet rs = ps.executeQuery();
			rs.next();
			return rs.getInt(1);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return -1;
	}

	public int getRem(int recid) {
		Connection conn = cf.getConnection();
		String sql = "select reimbursment from request where requestid=?";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ps.setInt(1, recid);
			ResultSet rs = ps.executeQuery();
			rs.next();
			return rs.getInt(1);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return -1;
	}

	@Override
	public int getApplicationStatus(int id) {
		Connection conn = cf.getConnection();
		String sql = "select status from request where requestid=?";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ps.setInt(1, id);
			ResultSet rs = ps.executeQuery();
			rs.next();
			return rs.getInt(1);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return -1;
	}

	public String getMyPending(int userID) {
		Connection conn = cf.getConnection();
		ArrayList<AppRequest> aar = new ArrayList<AppRequest>();
		String sql = "select * from request where request.userid=? and flagged = -1 order by eventdate";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ps.setInt(1, userID);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {

				aar.add(new AppRequest(rs.getInt(2), rs.getString(3), rs.getString(4),
						this.formatDate(rs.getDate(5).toString()), rs.getString(6), rs.getInt(7), rs.getInt(8),
						rs.getString(9), rs.getString(10), rs.getDouble(11), rs.getDouble(12), rs.getInt(1),
						rs.getString(14),rs.getString(17),rs.getInt(15),rs.getString(16), rs.getInt(13)));
			}
			String table = this.basicPendingTableToHtmlString(aar);
			return table;
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return null;
	}
	
	

	public String getMyPendingNotifications(int userID) {
		Connection conn = cf.getConnection();
		ArrayList<AppRequest> aar = new ArrayList<AppRequest>();
		String sql = "select * from request where request.userid=? and flagged = 0 and status != -1 order by eventdate";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ps.setInt(1, userID);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {

				aar.add(new AppRequest(rs.getInt(2), rs.getString(3), rs.getString(4),
						this.formatDate(rs.getDate(5).toString()), rs.getString(6), rs.getInt(7), rs.getInt(8),
						rs.getString(9), rs.getString(10), rs.getDouble(11), rs.getDouble(12), rs.getInt(1),
						rs.getString(14),rs.getString(17),rs.getInt(15),rs.getString(16), rs.getInt(13)));
			}
			String table = this.basicMoreInfoTableToHtmlString(aar);
			return table;
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return null;
	}

	public String getMyApproved(int userID) {
		Connection conn = cf.getConnection();
		ArrayList<AppRequest> aar = new ArrayList<AppRequest>();
		String sql = "select * from request where request.userid=? and flagged = -1 and status = 3 order by eventdate";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ps.setInt(1, userID);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {

				aar.add(new AppRequest(rs.getInt(2), rs.getString(3), rs.getString(4),
						this.formatDate(rs.getDate(5).toString()), rs.getString(6), rs.getInt(7), rs.getInt(8),
						rs.getString(9), rs.getString(10), rs.getDouble(11), rs.getDouble(12), rs.getInt(1),
						rs.getString(14),rs.getString(17),rs.getInt(15),rs.getString(16), rs.getInt(13)));
			}
			String table = this.basicApprovedTableToHtmlString(aar);
			return table;
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return null;
	}

	@Override
	public String getPendingSuper(int userID) {
		Connection conn = cf.getConnection();
		ArrayList<AppRequest> aar = new ArrayList<AppRequest>();
		String sql = "select * from request join employee on request.userid=employee.userid where status = 0 and employee.reportsto=? and flagged = -1 order by eventdate";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ps.setInt(1, userID);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				aar.add(new AppRequest(rs.getInt(2), rs.getString(3), rs.getString(4),
						this.formatDate(rs.getDate(5).toString()), rs.getString(6), rs.getInt(7), rs.getInt(8),
						rs.getString(9), rs.getString(10), rs.getDouble(11), rs.getDouble(12), rs.getInt(1),
						rs.getString(14),rs.getString(17),rs.getInt(15),rs.getString(16), rs.getInt(13)));
			}
			String table = this.pendingTableToHtmlString(aar);
			return table;
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return null;
	}

	public String getFlaggedSuper(int userID) {
		Connection conn = cf.getConnection();
		ArrayList<AppRequest> aar = new ArrayList<AppRequest>();
		String sql = "select * from request join employee on request.userid=employee.userid where status != -1 and employee.reportsto=? and flagged = 1 order by eventdate";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ps.setInt(1, userID);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				aar.add(new AppRequest(rs.getInt(2), rs.getString(3), rs.getString(4),
						this.formatDate(rs.getDate(5).toString()), rs.getString(6), rs.getInt(7), rs.getInt(8),
						rs.getString(9), rs.getString(10), rs.getDouble(11), rs.getDouble(12), rs.getInt(1),
						rs.getString(14),rs.getString(17),rs.getInt(15),rs.getString(16), rs.getInt(13)));
			}
			String table = this.pendingNotifToHtmlString(aar);
			
			return table;
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return null;
	}

	@Override
	public String getPendingDeptHead(int deptID) {
		Connection conn = cf.getConnection();
		ArrayList<AppRequest> aar = new ArrayList<AppRequest>();
		String sql = "select * from request join employee on request.userid=employee.userid where status = 1 and employee.department=? and flagged = -1 order by eventdate";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ps.setInt(1, deptID);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				aar.add(new AppRequest(rs.getInt(2), rs.getString(3), rs.getString(4),
						this.formatDate(rs.getDate(5).toString()), rs.getString(6), rs.getInt(7), rs.getInt(8),
						rs.getString(9), rs.getString(10), rs.getDouble(11), rs.getDouble(12), rs.getInt(1),
						rs.getString(14),rs.getString(17),rs.getInt(15),rs.getString(16), rs.getInt(13)));
			}
			String table = this.pendingDeptTableToHtmlString(aar);
			return table;
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return null;
	}

	public String getFlaggedDeptHead(int deptID) {
		Connection conn = cf.getConnection();
		ArrayList<AppRequest> aar = new ArrayList<AppRequest>();
		String sql = "select * from request join employee on request.userid=employee.userid where status != -1 and employee.department=? and flagged = 2 order by eventdate";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ps.setInt(1, deptID);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				aar.add(new AppRequest(rs.getInt(2), rs.getString(3), rs.getString(4),
						this.formatDate(rs.getDate(5).toString()), rs.getString(6), rs.getInt(7), rs.getInt(8),
						rs.getString(9), rs.getString(10), rs.getDouble(11), rs.getDouble(12), rs.getInt(1),
						rs.getString(14),rs.getString(17),rs.getInt(15),rs.getString(16), rs.getInt(13)));
			}
			String table = this.pendingDeptNotifToHtmlString(aar);
			
			return table;
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return null;
	}

	@Override
	public String getPendingBenCo() {
		Connection conn = cf.getConnection();
		ArrayList<AppRequest> aar = new ArrayList<AppRequest>();
		String sql = "select * from request where status = 2 and flagged = -1 order by eventdate";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				aar.add(new AppRequest(rs.getInt(2), rs.getString(3), rs.getString(4),
						this.formatDate(rs.getDate(5).toString()), rs.getString(6), rs.getInt(7), rs.getInt(8),
						rs.getString(9), rs.getString(10), rs.getDouble(11), rs.getDouble(12), rs.getInt(1),
						rs.getString(14),rs.getString(17),rs.getInt(15),rs.getString(16), rs.getInt(13)));
			}
			String table = this.pendingTableToHtmlString(aar);
			return table;
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return null;
	}

	private String approvedTableToHtmlString(ArrayList<AppRequest> aar) {
		StringBuffer sb = new StringBuffer();
		sb.append(
				"<!DOCTYPE html><html><head><meta charset='UTF-8'><style type='text/css'>ul {list-style-type: none;margin: 0;padding: 0;overflow: hidden;background-color: #333;}li {float: left;}li a {display: block;color: white;text-align: center;padding: 14px 16px;text-decoration: none;}li a:hover {background-color: #111;}.supHome:hover {background-color: #111;}.supHome {background-color: #333;border: none;color: white;padding: 15px 32px;text-align: center;text-decoration: none;display: inline-block;font-size: 16px;margin: 4px 2px;cursor: pointer;font-family: Verdana, Geneva, Tahoma, sans-serif;}table{border-collapse: collapse;}table tr td{border: 1px solid black;}th{background-color: gray;color: white;border: 1px solid black;}tr:nth-child(even) {background-color: #d3d3d3;}.approve{color: green;font-weight: bold;}.deny{color: red;font-weight: bold;}</style><title>Pending</title><link href=\"data:image/png;base64,\" rel=\"icon\" type=\"image/x-icon\"/></head><body><ul>\r\n" + 
				"		<li><a class=\"supHome\" href=\"supHome.html\">Home</a></li>\r\n" + 
				"		<li>\r\n" + 
				"			<form method=\"post\" action=\"supHome\">\r\n" + 
				"				<button class=\"supHome\" type=\"submit\" onclick=getTables()>Approve/Deny\r\n" + 
				"					Requests</button>\r\n" + 
				"			</form>\r\n" + 
				"		</li>\r\n" + 
				"		<li>\r\n" + 
				"			<form method=\"post\" action=\"notification\">\r\n" + 
				"				<button class=\"supHome\" type=\"submit\" onclick=getNotifTables()>View\r\n" + 
				"					Notifications</button>\r\n" + 
				"			</form>\r\n" + 
				"		</li>\r\n" + 
				"		<li>\r\n" + 
				"			<form method=\"post\" action=\"payout\">\r\n" + 
				"				<button class=\"supHome\" type=\"submit\" onclick=getPayTables()>Pay Out</button>\r\n" + 
				"			</form>\r\n" + 
				"		</li>\r\n" + 
				"		<li>\r\n" + 
				"			<form method=\"get\" action=\"logout\">\r\n" + 
				"				<button class=\"supHome\" onclick=logout()>Logout</button>\r\n" + 
				"			</form>\r\n" + 
				"		</li>\r\n" + 
				"	</ul><div><table id=\"table\">");
		sb.append("<thead>\r\n"
				+ "        <th>ReqID</th><th>UsrID</th><th>Event</th><th>Location</th><th>Date</th><th>Description</th><th>EventType/Category</th><th>Grading Scale</th><th>Passing Grade</th><th>Justification</th><th>Cost</th><th>Reimb.</th><th>Status</th><th>Attachment Links</th><th>More Info</th><th>Final Grade</th><th>Approve/Deny</th>"
				+ "    </thead>");
		sb.append("<tbody id=\"tbody\">");
		for (AppRequest a : aar) {
			
			sb.append("<tr><td>" + (a.getRequestid()) + "</td>");
			sb.append("<td>" + (a.getUserID()) + "</td>");
			sb.append("<td>" + (a.getName()) + "</td>");
			sb.append("<td>" + (a.getLocation()) + "</td>");
			sb.append("<td>" + (a.getDate()) + "</td>");
			sb.append("<td>" + (a.getDescription()) + "</td>");
			sb.append("<td>" + (h.getRetype().get(a.getType())) + "</td>");
			sb.append("<td>" + (h.getRgscale().get(a.getGradingScale())) + "</td>");
			sb.append("<td>" + (a.getPassingGrade()) + "</td>");
			sb.append("<td>" + (a.getJustification()) + "</td>");
			sb.append("<td>" + (a.getCost()) + "</td>");
			sb.append("<td>" + (a.getReimbursement()) + "</td>");
			sb.append("<td>" + (h.getStype().get(a.getStatus())) + "</td>");
			sb.append("<td>" + (a.getLinks()) + "</td>");
			sb.append("<td>" + (a.getMoreInfo()) + "</td>");
			sb.append("<td>" + (a.getFinalGrade()) + "</td>");
			sb.append(
					"<td><form method=\"post\" action=\"approve\"><button class=\"approve\" name=\"requestId\" value=");
			sb.append("\"" + a.getRequestid()
					+ "\" onclick= approve(value)>Approve &#9989;</button></form><form method=\"post\" action=\"deny\"><button class=\"deny\" name=\"requestId\" value=\"");
			sb.append(a.getRequestid() + "\" onclick=\"deny(value)\">Deny &#10060;</button></form></td>");
		}
		sb.append("</tbody></table></div>"  + "</body>");
		sb.append(
				"<script>window.onload = function() {let table = document.getElementById(\"tbody\");console.log(\"in window onload function\");for (var i = 0, row; row = table.rows[i]; i++){let tableDate = row.cells[4].innerHTML;tableDate = new Date(tableDate);tableDate.setFullYear(Math.abs(tableDate.getFullYear()));let curDate = new Date();curDate.setHours(0,0,0,0);if (tableDate-curDate <= 86400000*14) {row.style.color=\"red\";}else{break;}}};");
		sb.append(
				"function approve(ReqID){ console.log(\"Approve\", ReqID); let xhr = new XMLHttpRequest(); xhr.onreadystatechange = function() { if(xhr.readyState == 4 && xhr.status == 200) { location.reload(true); } };console.log(\"requestId=\"+ReqID); xhr.open(\"POST\",\"approve\", true); xhr.send(\"requestId=\" + ReqID); }");
		sb.append(
				"function deny(ReqID){ console.log(\"Deny\", ReqID); let xhr = new XMLHttpRequest(); xhr.onreadystatechange = function() { if(xhr.readyState == 4 && xhr.status == 200) { location.reload(true); } };console.log(\"requestId=\"+ReqID); xhr.open(\"POST\",\"deny\", true); xhr.send(\"requestId=\" + ReqID); }");
		sb.append(
				"function moreInfo() { let xhr = new XMLHttpRequest(); xhr.onreadystatechange = function() { if (xhr.readyState == 4 && xhr.status == 200) { location.reload(true); } }; xhr.open(\"POST\", \"moreInfo\", true); xhr.send(\"requestId=\" + document.getElementById(\"recid\").value + \"&& authority=\" + document.getElementById(\"basicEmployee\").value); }");

		sb.append("</script>");
		sb.append("</html>");

		return sb.toString();
	}
	//NEEDS TO BE DONE $$$$$$$
	public String getApprovedBenCo() {
		Connection conn = cf.getConnection();
		ArrayList<AppRequest> aar = new ArrayList<AppRequest>();
		String sql = "select * from request where status = 4 and gradingscale != 4 order by eventdate";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				aar.add(new AppRequest(rs.getInt(2), rs.getString(3), rs.getString(4),
						this.formatDate(rs.getDate(5).toString()), rs.getString(6), rs.getInt(7), rs.getInt(8),
						rs.getString(9), rs.getString(10), rs.getDouble(11), rs.getDouble(12), rs.getInt(1),
						rs.getString(14),rs.getString(17),rs.getInt(15),rs.getString(16), rs.getInt(13)));
			}
			String table = this.approvedTableToHtmlString(aar);
			return table;
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return null;
	}
	public String getApprovedSup() {
		Connection conn = cf.getConnection();
		ArrayList<AppRequest> aar = new ArrayList<AppRequest>();
		String sql = "select * from request where status = 4 and gradingscale = 4 order by eventdate";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				aar.add(new AppRequest(rs.getInt(2), rs.getString(3), rs.getString(4),
						this.formatDate(rs.getDate(5).toString()), rs.getString(6), rs.getInt(7), rs.getInt(8),
						rs.getString(9), rs.getString(10), rs.getDouble(11), rs.getDouble(12), rs.getInt(1),
						rs.getString(14),rs.getString(17),rs.getInt(15),rs.getString(16), rs.getInt(13)));
			}
			String table = this.approvedTableToHtmlString(aar);
			return table;
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return null;
	}

	private String pendingTableToHtmlString(ArrayList<AppRequest> aar) {
		StringBuffer sb = new StringBuffer();
		sb.append(
				"<!DOCTYPE html><html><head><meta charset='UTF-8'><style type='text/css'>ul {list-style-type: none;margin: 0;padding: 0;overflow: hidden;background-color: #333;}li {float: left;}li a {display: block;color: white;text-align: center;padding: 14px 16px;text-decoration: none;}li a:hover {background-color: #111;}.supHome:hover {background-color: #111;}.supHome {background-color: #333;border: none;color: white;padding: 15px 32px;text-align: center;text-decoration: none;display: inline-block;font-size: 16px;margin: 4px 2px;cursor: pointer;font-family: Verdana, Geneva, Tahoma, sans-serif;}table{border-collapse: collapse;}table tr td{border: 1px solid black;}th{background-color: gray;color: white;border: 1px solid black;}tr:nth-child(even) {background-color: #d3d3d3;}.approve{color: green;font-weight: bold;}.deny{color: red;font-weight: bold;}</style><title>Pending</title><link href=\"data:image/png;base64,\" rel=\"icon\" type=\"image/x-icon\"/></head><body><ul>\r\n" + 
				"		<li><a class=\"supHome\" href=\"supHome.html\">Home</a></li>\r\n" + 
				"		<li>\r\n" + 
				"			<form method=\"post\" action=\"supHome\">\r\n" + 
				"				<button class=\"supHome\" type=\"submit\" onclick=getTables()>Approve/Deny\r\n" + 
				"					Requests</button>\r\n" + 
				"			</form>\r\n" + 
				"		</li>\r\n" + 
				"		<li>\r\n" + 
				"			<form method=\"post\" action=\"notification\">\r\n" + 
				"				<button class=\"supHome\" type=\"submit\" onclick=getNotifTables()>View\r\n" + 
				"					Notifications</button>\r\n" + 
				"			</form>\r\n" + 
				"		</li>\r\n" + 
				"		<li>\r\n" + 
				"			<form method=\"post\" action=\"payout\">\r\n" + 
				"				<button class=\"supHome\" type=\"submit\" onclick=getPayTables()>Pay Out</button>\r\n" + 
				"			</form>\r\n" + 
				"		</li>\r\n" + 
				"		<li>\r\n" + 
				"			<form method=\"get\" action=\"logout\">\r\n" + 
				"				<button class=\"supHome\" onclick=logout()>Logout</button>\r\n" + 
				"			</form>\r\n" + 
				"		</li>\r\n" + 
				"	</ul><div><table id=\"table\">");
		sb.append("<thead>\r\n"
				+ "        <th>ReqID</th><th>UsrID</th><th>Event</th><th>Location</th><th>Date</th><th>Description</th><th>EventType/Category</th><th>Grading Scale</th><th>Passing Grade</th><th>Justification</th><th>Cost</th><th>Reimb.</th><th>Status</th><th>Attachment Links</th><th>More Info</th><th>Approve/Deny</th>"
				+ "    </thead>");
		sb.append("<tbody id=\"tbody\">");
		for (AppRequest a : aar) {
			
			sb.append("<tr><td>" + (a.getRequestid()) + "</td>");
			sb.append("<td>" + (a.getUserID()) + "</td>");
			sb.append("<td>" + (a.getName()) + "</td>");
			sb.append("<td>" + (a.getLocation()) + "</td>");
			sb.append("<td>" + (a.getDate()) + "</td>");
			sb.append("<td>" + (a.getDescription()) + "</td>");
			sb.append("<td>" + (h.getRetype().get(a.getType())) + "</td>");
			sb.append("<td>" + (h.getRgscale().get(a.getGradingScale())) + "</td>");
			sb.append("<td>" + (a.getPassingGrade()) + "</td>");
			sb.append("<td>" + (a.getJustification()) + "</td>");
			sb.append("<td>" + (a.getCost()) + "</td>");
			sb.append("<td>" + (a.getReimbursement()) + "</td>");
			sb.append("<td>" + (h.getStype().get(a.getStatus())) + "</td>");
			sb.append("<td>" + (a.getLinks()) + "</td>");
			sb.append("<td>" + (a.getMoreInfo()) + "</td>");
			sb.append(
					"<td><form method=\"post\" action=\"approve\"><button class=\"approve\" name=\"requestId\" value=");
			sb.append("\"" + a.getRequestid()
					+ "\" onclick= approve(value)>Approve &#9989;</button></form><form method=\"post\" action=\"deny\"><button class=\"deny\" name=\"requestId\" value=\"");
			sb.append(a.getRequestid() + "\" onclick=\"deny(value)\">Deny &#10060;</button></form></td>");
		}
		sb.append("</tbody></table></div>" + "<div>\r\n" + " <form method=\"post\" action=\"askmoreinfo\">\r\n"
				+ "            <p>Request More Info</p>\r\n"
				+ "            <input type=\"number\" min = 1 id=\"recid\" name=\"recind\" placeholder=\"Insert Request Id\" />\r\n"
				+ "            \r\n"
				+ "                <input type = \"submit\" class=\"moreInfo\" name=\"basicEmployee\" id=\"basicEmployee\" value=\"Employee\" onClick=\"moreInfo()\">\r\n"
				+ "            \r\n"
				+ "                <input type = \"submit\" name=\"DirectSuper\" class=\"moreInfo\" id=\"DirectSuper\" value=\"DirectSupervisor\" onClick=\"moreInfo()\">\r\n"
				+ "           <input type = \"submit\" name=\"DepartmentHead\" class=\"moreInfo\" id=\"DepartmentHead\" value=\"DepartmentHead\" onClick=\"moreInfo()\">\r\n"
				+ "        </form>" + "    </div>" + "</body>");
		sb.append(
				"<script>window.onload = function() {let table = document.getElementById(\"tbody\");console.log(\"in window onload function\");for (var i = 0, row; row = table.rows[i]; i++){let tableDate = row.cells[4].innerHTML;tableDate = new Date(tableDate);tableDate.setFullYear(Math.abs(tableDate.getFullYear()));let curDate = new Date();curDate.setHours(0,0,0,0);if (tableDate-curDate <= 86400000*14) {row.style.color=\"red\";}else{break;}}};");
		sb.append(
				"function approve(ReqID){ console.log(\"Approve\", ReqID); let xhr = new XMLHttpRequest(); xhr.onreadystatechange = function() { if(xhr.readyState == 4 && xhr.status == 200) { location.reload(true); } };console.log(\"requestId=\"+ReqID); xhr.open(\"POST\",\"approve\", true); xhr.send(\"requestId=\" + ReqID); }");
		sb.append(
				"function deny(ReqID){ console.log(\"Deny\", ReqID); let xhr = new XMLHttpRequest(); xhr.onreadystatechange = function() { if(xhr.readyState == 4 && xhr.status == 200) { location.reload(true); } };console.log(\"requestId=\"+ReqID); xhr.open(\"POST\",\"deny\", true); xhr.send(\"requestId=\" + ReqID); }");
		sb.append(
				"function moreInfo() { let xhr = new XMLHttpRequest(); xhr.onreadystatechange = function() { if (xhr.readyState == 4 && xhr.status == 200) { location.reload(true); } }; xhr.open(\"POST\", \"moreInfo\", true); xhr.send(\"requestId=\" + document.getElementById(\"recid\").value + \"&& authority=\" + document.getElementById(\"basicEmployee\").value); }");

		sb.append("</script>");
		sb.append("</html>");

		return sb.toString();
	}
	private String pendingDeptTableToHtmlString(ArrayList<AppRequest> aar) {
		StringBuffer sb = new StringBuffer();
		sb.append(
				"<!DOCTYPE html><html><head><meta charset='UTF-8'><style type='text/css'>ul {list-style-type: none;margin: 0;padding: 0;overflow: hidden;background-color: #333;}li {float: left;}li a {display: block;color: white;text-align: center;padding: 14px 16px;text-decoration: none;}li a:hover {background-color: #111;}.supHome:hover {background-color: #111;}.supHome {background-color: #333;border: none;color: white;padding: 15px 32px;text-align: center;text-decoration: none;display: inline-block;font-size: 16px;margin: 4px 2px;cursor: pointer;font-family: Verdana, Geneva, Tahoma, sans-serif;}table{border-collapse: collapse;}table tr td{border: 1px solid black;}th{background-color: gray;color: white;border: 1px solid black;}tr:nth-child(even) {background-color: #d3d3d3;}.approve{color: green;font-weight: bold;}.deny{color: red;font-weight: bold;}</style><title>Pending</title><link href=\"data:image/png;base64,\" rel=\"icon\" type=\"image/x-icon\"/></head><body><ul> <li><a class=\"supHome\" href=\"DeptHome.html\">Home</a></li> <li> <form method=\"post\" action=\"supHome\"> <button class=\"supHome\" type=\"submit\" onclick=getTables()>Approve/Deny Requests</button> </form> </li> <li> <form method=\"post\" action=\"notification\"> <button class=\"supHome\" type=\"submit\" onclick=getNotifTables()>View Notifications </button> </form> </li> <li> <form method=\"get\" action=\"logout\"> <button class=\"supHome\" onclick=logout()>Logout</button> </form> </li> </ul><div><table id=\"table\">");
		sb.append("<thead>\r\n"
				+ "        <th>ReqID</th><th>UsrID</th><th>Event</th><th>Location</th><th>Date</th><th>Description</th><th>EventType/Category</th><th>Grading Scale</th><th>Passing Grade</th><th>Justification</th><th>Cost</th><th>Reimb.</th><th>Status</th><th>Attachment Links</th><th>More Info</th><th>Approve/Deny</th>"
				+ "    </thead>");
		sb.append("<tbody id=\"tbody\">");
		for (AppRequest a : aar) {
			
			sb.append("<tr><td>" + (a.getRequestid()) + "</td>");
			sb.append("<td>" + (a.getUserID()) + "</td>");
			sb.append("<td>" + (a.getName()) + "</td>");
			sb.append("<td>" + (a.getLocation()) + "</td>");
			sb.append("<td>" + (a.getDate()) + "</td>");
			sb.append("<td>" + (a.getDescription()) + "</td>");
			sb.append("<td>" + (h.getRetype().get(a.getType())) + "</td>");
			sb.append("<td>" + (h.getRgscale().get(a.getGradingScale())) + "</td>");
			sb.append("<td>" + (a.getPassingGrade()) + "</td>");
			sb.append("<td>" + (a.getJustification()) + "</td>");
			sb.append("<td>" + (a.getCost()) + "</td>");
			sb.append("<td>" + (a.getReimbursement()) + "</td>");
			sb.append("<td>" + (h.getStype().get(a.getStatus())) + "</td>");
			sb.append("<td>" + (a.getLinks()) + "</td>");
			sb.append("<td>" + (a.getMoreInfo()) + "</td>");
			sb.append(
					"<td><form method=\"post\" action=\"approve\"><button class=\"approve\" name=\"requestId\" value=");
			sb.append("\"" + a.getRequestid()
					+ "\" onclick= approve(value)>Approve &#9989;</button></form><form method=\"post\" action=\"deny\"><button class=\"deny\" name=\"requestId\" value=\"");
			sb.append(a.getRequestid() + "\" onclick=\"deny(value)\">Deny &#10060;</button></form></td>");
		}
		sb.append("</tbody></table></div>" + "<div>\r\n" + " <form method=\"post\" action=\"askmoreinfo\">\r\n"
				+ "            <p>Request More Info</p>\r\n"
				+ "            <input type=\"number\" min = 1 id=\"recid\" name=\"recind\" placeholder=\"Insert Request Id\" />\r\n"
				+ "            \r\n"
				+ "                <input type = \"submit\" class=\"moreInfo\" name=\"basicEmployee\" id=\"basicEmployee\" value=\"Employee\" onClick=\"moreInfo()\">\r\n"
				+ "            \r\n"
				+ "                <input type = \"submit\" name=\"DirectSuper\" class=\"moreInfo\" id=\"DirectSuper\" value=\"DirectSupervisor\" onClick=\"moreInfo()\">\r\n"
				+ "           <input type = \"submit\" name=\"DepartmentHead\" class=\"moreInfo\" id=\"DepartmentHead\" value=\"DepartmentHead\" onClick=\"moreInfo()\">\r\n"
				+ "        </form>" + "    </div>" + "</body>");
		sb.append(
				"<script>window.onload = function() {let table = document.getElementById(\"tbody\");console.log(\"in window onload function\");for (var i = 0, row; row = table.rows[i]; i++){let tableDate = row.cells[4].innerHTML;tableDate = new Date(tableDate);tableDate.setFullYear(Math.abs(tableDate.getFullYear()));let curDate = new Date();curDate.setHours(0,0,0,0);if (tableDate-curDate <= 86400000*14) {row.style.color=\"red\";}else{break;}}};");
		sb.append(
				"function approve(ReqID){ console.log(\"Approve\", ReqID); let xhr = new XMLHttpRequest(); xhr.onreadystatechange = function() { if(xhr.readyState == 4 && xhr.status == 200) { location.reload(true); } };console.log(\"requestId=\"+ReqID); xhr.open(\"POST\",\"approve\", true); xhr.send(\"requestId=\" + ReqID); }");
		sb.append(
				"function deny(ReqID){ console.log(\"Deny\", ReqID); let xhr = new XMLHttpRequest(); xhr.onreadystatechange = function() { if(xhr.readyState == 4 && xhr.status == 200) { location.reload(true); } };console.log(\"requestId=\"+ReqID); xhr.open(\"POST\",\"deny\", true); xhr.send(\"requestId=\" + ReqID); }");
		sb.append(
				"function moreInfo() { let xhr = new XMLHttpRequest(); xhr.onreadystatechange = function() { if (xhr.readyState == 4 && xhr.status == 200) { location.reload(true); } }; xhr.open(\"POST\", \"moreInfo\", true); xhr.send(\"requestId=\" + document.getElementById(\"recid\").value + \"&& authority=\" + document.getElementById(\"basicEmployee\").value); }");

		sb.append("</script>");
		sb.append("</html>");

		return sb.toString();
	}
	
	private String pendingNotifToHtmlString(ArrayList<AppRequest> aar) {
		StringBuffer sb = new StringBuffer();
		sb.append(
				"<!DOCTYPE html><html><head><meta charset='UTF-8'><style type='text/css'>ul {list-style-type: none;margin: 0;padding: 0;overflow: hidden;background-color: #333;}li {float: left;}li a {display: block;color: white;text-align: center;padding: 14px 16px;text-decoration: none;}li a:hover {background-color: #111;}.supHome:hover {background-color: #111;}.supHome {background-color: #333;border: none;color: white;padding: 15px 32px;text-align: center;text-decoration: none;display: inline-block;font-size: 16px;margin: 4px 2px;cursor: pointer;font-family: Verdana, Geneva, Tahoma, sans-serif;}table{border-collapse: collapse;}table tr td{border: 1px solid black;}th{background-color: gray;color: white;border: 1px solid black;}tr:nth-child(even) {background-color: #d3d3d3;}.approve{color: green;font-weight: bold;}.deny{color: red;font-weight: bold;}</style><title>Pending</title><link href=\"data:image/png;base64,\" rel=\"icon\" type=\"image/x-icon\"/></head><body><ul>\r\n" + 
				"		<li><a class=\"supHome\" href=\"supHome.html\">Home</a></li>\r\n" + 
				"		<li>\r\n" + 
				"			<form method=\"post\" action=\"supHome\">\r\n" + 
				"				<button class=\"supHome\" type=\"submit\" onclick=getTables()>Approve/Deny\r\n" + 
				"					Requests</button>\r\n" + 
				"			</form>\r\n" + 
				"		</li>\r\n" + 
				"		<li>\r\n" + 
				"			<form method=\"post\" action=\"notification\">\r\n" + 
				"				<button class=\"supHome\" type=\"submit\" onclick=getNotifTables()>View\r\n" + 
				"					Notifications</button>\r\n" + 
				"			</form>\r\n" + 
				"		</li>\r\n" + 
				"		<li>\r\n" + 
				"			<form method=\"post\" action=\"payout\">\r\n" + 
				"				<button class=\"supHome\" type=\"submit\" onclick=getPayTables()>Pay Out</button>\r\n" + 
				"			</form>\r\n" + 
				"		</li>\r\n" + 
				"		<li>\r\n" + 
				"			<form method=\"get\" action=\"logout\">\r\n" + 
				"				<button class=\"supHome\" onclick=logout()>Logout</button>\r\n" + 
				"			</form>\r\n" + 
				"		</li>\r\n" + 
				"	</ul><div><table id=\"table\">");
		sb.append("<thead>\r\n"
				+ "        <th>ReqID</th><th>UsrID</th><th>Event</th><th>Location</th><th>Date</th><th>Description</th><th>EventType/Category</th><th>Grading Scale</th><th>Passing Grade</th><th>Justification</th><th>Cost</th><th>Reimb.</th><th>Status</th><th>Attachment Links</th><th>More Info</th><th>Submit More Info</th>"
				+ "    </thead>");
		sb.append("<tbody id=\"tbody\">");
		for (AppRequest a : aar) {
			sb.append("<tr><td>" + (a.getRequestid()) + "</td>");
			sb.append("<td>" + (a.getUserID()) + "</td>");
			sb.append("<td>" + (a.getName()) + "</td>");
			sb.append("<td>" + (a.getLocation()) + "</td>");
			sb.append("<td>" + (a.getDate()) + "</td>");
			sb.append("<td>" + (a.getDescription()) + "</td>");
			sb.append("<td>" + (h.getRetype().get(a.getType())) + "</td>");
			sb.append("<td>" + (h.getRgscale().get(a.getGradingScale())) + "</td>");
			sb.append("<td>" + (a.getPassingGrade()) + "</td>");
			sb.append("<td>" + (a.getJustification()) + "</td>");
			sb.append("<td>" + (a.getCost()) + "</td>");
			sb.append("<td>" + (a.getReimbursement()) + "</td>");
			sb.append("<td>" + (h.getStype().get(a.getStatus())) + "</td>");
			sb.append("<td>" + (a.getLinks()) + "</td>");
			sb.append("<td>" + (a.getMoreInfo()) + "</td>");
			sb.append("<td><form method=\"post\" action=\"submitmoreinfo\">"
					+ "<input type=\"text\" placeholder = \"Add more info\" name=\"info\"/>");
			sb.append("<button class = \"moreinfo\" name = \"requestId\" value =\"");
			sb.append(a.getRequestid() + "\" onclick=\"submitinfo(value)\">&#9989</button></form></td></tr>");
		}
		sb.append("</tbody></table></div></body>");
		sb.append(
				"<script>window.onload = function() {let table = document.getElementById(\"tbody\");console.log(\"in window onload function\");for (var i = 0, row; row = table.rows[i]; i++){let tableDate = row.cells[4].innerHTML;tableDate = new Date(tableDate);tableDate.setFullYear(Math.abs(tableDate.getFullYear()));let curDate = new Date();curDate.setHours(0,0,0,0);if (tableDate-curDate <= 86400000*14) {row.style.color=\"red\";}else{break;}}};");
		sb.append(
				"function submitinfo(ReqID){ console.log(\"submitinfo\", ReqID); let xhr = new XMLHttpRequest(); xhr.onreadystatechange = function() { if(xhr.readyState == 4 && xhr.status == 200) { location.reload(true); } };console.log(\"requestId=\"+ReqID); xhr.open(\"POST\",\"submitmoreinfo\", true); xhr.send(\"requestId=\" + ReqID); }");
		sb.append("</script>");
		sb.append("</html>");

		return sb.toString();
	}
	private String pendingDeptNotifToHtmlString(ArrayList<AppRequest> aar) {
		StringBuffer sb = new StringBuffer();
		sb.append(
				"<!DOCTYPE html><html><head><meta charset='UTF-8'><style type='text/css'>ul {list-style-type: none;margin: 0;padding: 0;overflow: hidden;background-color: #333;}li {float: left;}li a {display: block;color: white;text-align: center;padding: 14px 16px;text-decoration: none;}li a:hover {background-color: #111;}.supHome:hover {background-color: #111;}.supHome {background-color: #333;border: none;color: white;padding: 15px 32px;text-align: center;text-decoration: none;display: inline-block;font-size: 16px;margin: 4px 2px;cursor: pointer;font-family: Verdana, Geneva, Tahoma, sans-serif;}table{border-collapse: collapse;}table tr td{border: 1px solid black;}th{background-color: gray;color: white;border: 1px solid black;}tr:nth-child(even) {background-color: #d3d3d3;}.approve{color: green;font-weight: bold;}.deny{color: red;font-weight: bold;}</style><title>Pending</title><link href=\"data:image/png;base64,\" rel=\"icon\" type=\"image/x-icon\"/></head><body><ul> <li><a class=\"supHome\" href=\"DeptHome.html\">Home</a></li> <li> <form method=\"post\" action=\"supHome\"> <button class=\"supHome\" type=\"submit\" onclick=getTables()>Approve/Deny Requests</button> </form> </li> <li> <form method=\"post\" action=\"notification\"> <button class=\"supHome\" type=\"submit\" onclick=getNotifTables()>View Notifications </button> </form> </li> <li> <form method=\"get\" action=\"logout\"> <button class=\"supHome\" onclick=logout()>Logout</button> </form> </li> </ul><div><table id=\"table\">");
		sb.append("<thead>\r\n"
				+ "        <th>ReqID</th><th>UsrID</th><th>Event</th><th>Location</th><th>Date</th><th>Description</th><th>EventType/Category</th><th>Grading Scale</th><th>Passing Grade</th><th>Justification</th><th>Cost</th><th>Reimb.</th><th>Status</th><th>Attachment Links</th><th>More Info</th><th>Submit More Info</th>"
				+ "    </thead>");
		sb.append("<tbody id=\"tbody\">");
		for (AppRequest a : aar) {
			sb.append("<tr><td>" + (a.getRequestid()) + "</td>");
			sb.append("<td>" + (a.getUserID()) + "</td>");
			sb.append("<td>" + (a.getName()) + "</td>");
			sb.append("<td>" + (a.getLocation()) + "</td>");
			sb.append("<td>" + (a.getDate()) + "</td>");
			sb.append("<td>" + (a.getDescription()) + "</td>");
			sb.append("<td>" + (h.getRetype().get(a.getType())) + "</td>");
			sb.append("<td>" + (h.getRgscale().get(a.getGradingScale())) + "</td>");
			sb.append("<td>" + (a.getPassingGrade()) + "</td>");
			sb.append("<td>" + (a.getJustification()) + "</td>");
			sb.append("<td>" + (a.getCost()) + "</td>");
			sb.append("<td>" + (a.getReimbursement()) + "</td>");
			sb.append("<td>" + (h.getStype().get(a.getStatus())) + "</td>");
			sb.append("<td>" + (a.getLinks()) + "</td>");
			sb.append("<td>" + (a.getMoreInfo()) + "</td>");
			sb.append("<td><form method=\"post\" action=\"submitmoreinfo\">"
					+ "<input type=\"text\" placeholder = \"Add more info\" name=\"info\"/>");
			sb.append("<button class = \"moreinfo\" name = \"requestId\" value =\"");
			sb.append(a.getRequestid() + "\" onclick=\"submitinfo(value)\">&#9989</button></form></td></tr>");
		}
		sb.append("</tbody></table></div></body>");
		sb.append(
				"<script>window.onload = function() {let table = document.getElementById(\"tbody\");console.log(\"in window onload function\");for (var i = 0, row; row = table.rows[i]; i++){let tableDate = row.cells[4].innerHTML;tableDate = new Date(tableDate);tableDate.setFullYear(Math.abs(tableDate.getFullYear()));let curDate = new Date();curDate.setHours(0,0,0,0);if (tableDate-curDate <= 86400000*14) {row.style.color=\"red\";}else{break;}}};");
		sb.append(
				"function submitinfo(ReqID){ console.log(\"submitinfo\", ReqID); let xhr = new XMLHttpRequest(); xhr.onreadystatechange = function() { if(xhr.readyState == 4 && xhr.status == 200) { location.reload(true); } };console.log(\"requestId=\"+ReqID); xhr.open(\"POST\",\"submitmoreinfo\", true); xhr.send(\"requestId=\" + ReqID); }");
		sb.append("</script>");
		sb.append("</html>");

		return sb.toString();
	}

	private String basicPendingTableToHtmlString(ArrayList<AppRequest> aar) {
		StringBuffer sb = new StringBuffer();
		sb.append(
				"<!DOCTYPE html><html><head><meta charset='UTF-8'><style type='text/css'>ul {list-style-type: none;margin: 0;padding: 0;overflow: hidden;background-color: #333;}li {float: left;}li a {display: block;color: white;text-align: center;padding: 14px 16px;text-decoration: none;}li a:hover {background-color: #111;}.supHome:hover {background-color: #111;}.supHome {background-color: #333;border: none;color: white;padding: 15px 32px;text-align: center;text-decoration: none;display: inline-block;font-size: 16px;margin: 4px 2px;cursor: pointer;font-family: Verdana, Geneva, Tahoma, sans-serif;}table{border-collapse: collapse;}table tr td{border: 1px solid black;}th{background-color: gray;color: white;border: 1px solid black;}tr:nth-child(even) {background-color: #d3d3d3;}.approve{color: green;font-weight: bold;}.deny{color: red;font-weight: bold;}</style><title>Pending</title><link href=\"data:image/png;base64,\" rel=\"icon\" type=\"image/x-icon\"/></head><body><ul><li><a class=\"supHome\" href=\"home.html\">Home</a></li><li><form method=\"post\" action=\"supHome\"><button class=\"supHome\" type=\"submit\" onclick=getTables()>View Pending Requests</button></form></li><li><form method=\"post\" action=\"notification\"><button class=\"supHome\" type=\"submit\" onclick=getNotifTables()>View Notifications</button></form></li><li><a class = \"supHome\" href=\"form.html\">Form</a></li>        <li>\r\n" + 
				"            <form method=\"post\" action=\"grades\">\r\n" + 
				"                <button class=\"supHome\" type=\"submit\" onclick=getGradesTable()>Submit\r\n" + 
				"                    Final Grades</button>\r\n" + 
				"            </form>\r\n" + 
				"        </li><li><form method=\"get\" action=\"logout\"><button class=\"supHome\" onclick=logout()>Logout</button></form></li></ul><div><table id=\"table\">");
		sb.append("<thead>\r\n"
				+ "        <th>Event</th><th>Location</th><th>Date</th><th>Description</th><th>EventType/Category</th><th>Grading Scale</th><th>Passing Grade</th><th>Justification</th><th>Cost</th><th>Reimb.</th><th>Status</th><th>Attachment Links</th><th>Reject</th>"
				+ "    </thead>");
		sb.append("<tbody id=\"tbody\">");
		for (AppRequest a : aar) {

			sb.append("<tr><td>" + (a.getName()) + "</td>");
			sb.append("<td>" + (a.getLocation()) + "</td>");
			sb.append("<td>" + (a.getDate()) + "</td>");
			sb.append("<td>" + (a.getDescription()) + "</td>");
			sb.append("<td>" + (h.getRetype().get(a.getType())) + "</td>");
			sb.append("<td>" + (h.getRgscale().get(a.getGradingScale())) + "</td>");
			sb.append("<td>" + (a.getPassingGrade()) + "</td>");
			sb.append("<td>" + (a.getJustification()) + "</td>");
			sb.append("<td>" + (a.getCost()) + "</td>");
			sb.append("<td>" + (a.getReimbursement()) + "</td>");
			sb.append("<td>" + (h.getStype().get(a.getStatus())) + "</td>");
			sb.append("<td>" + (a.getLinks()) + "</td>");
			sb.append("<td><form method=\"post\" action=\"deny\"><button class=\"deny\" name=\"requestId\" value=\"");
			sb.append(a.getRequestid() + "\" onclick=\"deny(value)\">Reject &#10060;</button></form></td></tr>");
		}
		sb.append("</tbody></table></div></body>");
		sb.append(
				"<script>window.onload = function() {let table = document.getElementById(\"tbody\");console.log(\"in window onload function\");for (var i = 0, row; row = table.rows[i]; i++){let tableDate = row.cells[4].innerHTML;tableDate = new Date(tableDate);tableDate.setFullYear(Math.abs(tableDate.getFullYear()));let curDate = new Date();curDate.setHours(0,0,0,0);if (tableDate-curDate <= 86400000*14) {row.style.color=\"red\";}else{break;}}};");
		sb.append(
				"function deny(ReqID){ console.log(\"Deny\", ReqID); let xhr = new XMLHttpRequest(); xhr.onreadystatechange = function() { if(xhr.readyState == 4 && xhr.status == 200) { location.reload(true); } };console.log(\"requestId=\"+ReqID); xhr.open(\"POST\",\"deny\", true); xhr.send(\"requestId=\" + ReqID); }");
		sb.append("</script>");
		sb.append("</html>");

		return sb.toString();
	}

	private String basicMoreInfoTableToHtmlString(ArrayList<AppRequest> aar) {
		StringBuffer sb = new StringBuffer();
		sb.append(
				"<!DOCTYPE html><html><head><meta charset='UTF-8'><style type='text/css'>ul {list-style-type: none;margin: 0;padding: 0;overflow: hidden;background-color: #333;}li {float: left;}li a {display: block;color: white;text-align: center;padding: 14px 16px;text-decoration: none;}li a:hover {background-color: #111;}.supHome:hover {background-color: #111;}.supHome {background-color: #333;border: none;color: white;padding: 15px 32px;text-align: center;text-decoration: none;display: inline-block;font-size: 16px;margin: 4px 2px;cursor: pointer;font-family: Verdana, Geneva, Tahoma, sans-serif;}table{border-collapse: collapse;}table tr td{border: 1px solid black;}th{background-color: gray;color: white;border: 1px solid black;}tr:nth-child(even) {background-color: #d3d3d3;}.approve{color: green;font-weight: bold;}.deny{color: red;font-weight: bold;}</style><title>Pending</title><link href=\"data:image/png;base64,\" rel=\"icon\" type=\"image/x-icon\"/></head><body><ul><li><a class=\"supHome\" href=\"home.html\">Home</a></li><li><form method=\"post\" action=\"supHome\"><button class=\"supHome\" type=\"submit\" onclick=getTables()>View Pending Requests</button></form></li><li><form method=\"post\" action=\"notification\"><button class=\"supHome\" type=\"submit\" onclick=getNotifTables()>View Notifications</button></form></li><li><a class = \"supHome\" href=\"form.html\">Form</a></li>        <li>\r\n" + 
				"            <form method=\"post\" action=\"grades\">\r\n" + 
				"                <button class=\"supHome\" type=\"submit\" onclick=getGradesTable()>Submit\r\n" + 
				"                    Final Grades</button>\r\n" + 
				"            </form>\r\n" + 
				"        </li><li><form method=\"get\" action=\"logout\"><button class=\"supHome\" onclick=logout()>Logout</button></form></li></ul><div><table id=\"table\">");
		sb.append("<thead>\r\n"
				+ "        <th>Event</th><th>Location</th><th>Date</th><th>Description</th><th>EventType/Category</th><th>Grading Scale</th><th>Passing Grade</th><th>Justification</th><th>Cost</th><th>Reimb.</th><th>Status</th><th>Attachment Links</th><th>More Info</th>"
				+ "    </thead>");
		sb.append("<tbody id=\"tbody\">");
		for (AppRequest a : aar) {

			sb.append("<tr><td>" + (a.getName()) + "</td>");
			sb.append("<td>" + (a.getLocation()) + "</td>");
			sb.append("<td>" + (a.getDate()) + "</td>");
			sb.append("<td>" + (a.getDescription()) + "</td>");
			sb.append("<td>" + (h.getRetype().get(a.getType())) + "</td>");
			sb.append("<td>" + (h.getRgscale().get(a.getGradingScale())) + "</td>");
			sb.append("<td>" + (a.getPassingGrade()) + "</td>");
			sb.append("<td>" + (a.getJustification()) + "</td>");
			sb.append("<td>" + (a.getCost()) + "</td>");
			sb.append("<td>" + (a.getReimbursement()) + "</td>");
			sb.append("<td>" + (h.getStype().get(a.getStatus())) + "</td>");
			sb.append("<td>" + (a.getLinks()) + "</td>");
			sb.append("<td><form method=\"post\" action=\"submitmoreinfo\">"
					+ "<input type=\"text\" placeholder = \"Add more info\" name=\"info\"/>");
			sb.append("<button class = \"moreinfo\" name = \"requestId\" value =\"");
			sb.append(a.getRequestid() + "\" onclick=\"submitinfo(value)\">&#9989</button></form></td></tr>");
		}
		sb.append("</tbody></table></div></body>");
		sb.append(
				"<script>window.onload = function() {let table = document.getElementById(\"tbody\");console.log(\"in window onload function\");for (var i = 0, row; row = table.rows[i]; i++){let tableDate = row.cells[4].innerHTML;tableDate = new Date(tableDate);tableDate.setFullYear(Math.abs(tableDate.getFullYear()));let curDate = new Date();curDate.setHours(0,0,0,0);if (tableDate-curDate <= 86400000*14) {row.style.color=\"red\";}else{break;}}};");
		sb.append(
				"function submitinfo(ReqID){ console.log(\"submitinfo\", ReqID); let xhr = new XMLHttpRequest(); xhr.onreadystatechange = function() { if(xhr.readyState == 4 && xhr.status == 200) { location.reload(true); } };console.log(\"requestId=\"+ReqID); xhr.open(\"POST\",\"submitmoreinfo\", true); xhr.send(\"requestId=\" + ReqID); }");
		sb.append("</script>");
		sb.append("</html>");

		return sb.toString();
	}
	
	private String basicApprovedTableToHtmlString(ArrayList<AppRequest> aar) {
		StringBuffer sb = new StringBuffer();
		sb.append(
				"<!DOCTYPE html><html><head><meta charset='UTF-8'><style type='text/css'>ul {list-style-type: none;margin: 0;padding: 0;overflow: hidden;background-color: #333;}li {float: left;}li a {display: block;color: white;text-align: center;padding: 14px 16px;text-decoration: none;}li a:hover {background-color: #111;}.supHome:hover {background-color: #111;}.supHome {background-color: #333;border: none;color: white;padding: 15px 32px;text-align: center;text-decoration: none;display: inline-block;font-size: 16px;margin: 4px 2px;cursor: pointer;font-family: Verdana, Geneva, Tahoma, sans-serif;}table{border-collapse: collapse;}table tr td{border: 1px solid black;}th{background-color: gray;color: white;border: 1px solid black;}tr:nth-child(even) {background-color: #d3d3d3;}.approve{color: green;font-weight: bold;}.deny{color: red;font-weight: bold;}</style><title>Pending</title><link href=\"data:image/png;base64,\" rel=\"icon\" type=\"image/x-icon\"/></head><body><ul><li><a class=\"supHome\" href=\"home.html\">Home</a></li><li><form method=\"post\" action=\"supHome\"><button class=\"supHome\" type=\"submit\" onclick=getTables()>View Pending Requests</button></form></li><li><form method=\"post\" action=\"notification\"><button class=\"supHome\" type=\"submit\" onclick=getNotifTables()>View Notifications</button></form></li><li><a class = \"supHome\" href=\"form.html\">Form</a></li>        <li>\r\n" + 
				"            <form method=\"post\" action=\"grades\">\r\n" + 
				"                <button class=\"supHome\" type=\"submit\" onclick=getGradesTable()>Submit\r\n" + 
				"                    Final Grades</button>\r\n" + 
				"            </form>\r\n" + 
				"        </li><li><form method=\"get\" action=\"logout\"><button class=\"supHome\" onclick=logout()>Logout</button></form></li></ul><div><table id=\"table\">");
		sb.append("<thead>\r\n"
				+ "        <th>Event</th><th>Location</th><th>Date</th><th>Description</th><th>EventType/Category</th><th>Grading Scale</th><th>Passing Grade</th><th>Justification</th><th>Cost</th><th>Reimb.</th><th>Status</th><th>Attachment Links</th><th>Submit Final Grade</th>"
				+ "    </thead>");
		sb.append("<tbody id=\"tbody\">");
		for (AppRequest a : aar) {

			sb.append("<tr><td>" + (a.getName()) + "</td>");
			sb.append("<td>" + (a.getLocation()) + "</td>");
			sb.append("<td>" + (a.getDate()) + "</td>");
			sb.append("<td>" + (a.getDescription()) + "</td>");
			sb.append("<td>" + (h.getRetype().get(a.getType())) + "</td>");
			sb.append("<td>" + (h.getRgscale().get(a.getGradingScale())) + "</td>");
			sb.append("<td>" + (a.getPassingGrade()) + "</td>");
			sb.append("<td>" + (a.getJustification()) + "</td>");
			sb.append("<td>" + (a.getCost()) + "</td>");
			sb.append("<td>" + (a.getReimbursement()) + "</td>");
			sb.append("<td>" + (h.getStype().get(a.getStatus())) + "</td>");
			sb.append("<td>" + (a.getLinks()) + "</td>");
			sb.append("<td><form method=\"post\" action=\"submitgrade\">"
					+ "<input type=\"text\" placeholder = \"Add Final Grade\" name=\"grade\"/>");
			sb.append("<button class = \"moreinfo\" name = \"requestId\" value =\"");
			sb.append(a.getRequestid() + "\" onclick=\"submitinfo(value)\">&#9989</button></form></td></tr>");
		}
		sb.append("</tbody></table></div></body>");
		sb.append(
				"<script>window.onload = function() {let table = document.getElementById(\"tbody\");console.log(\"in window onload function\");for (var i = 0, row; row = table.rows[i]; i++){let tableDate = row.cells[4].innerHTML;tableDate = new Date(tableDate);tableDate.setFullYear(Math.abs(tableDate.getFullYear()));let curDate = new Date();curDate.setHours(0,0,0,0);if (tableDate-curDate <= 86400000*14) {row.style.color=\"red\";}else{break;}}};");
		sb.append(
				"function submitgrade(ReqID){ console.log(\"submitgrade\", ReqID); let xhr = new XMLHttpRequest(); xhr.onreadystatechange = function() { if(xhr.readyState == 4 && xhr.status == 200) { location.reload(true); } };console.log(\"requestId=\"+ReqID); xhr.open(\"POST\",\"submitgrade\", true); xhr.send(\"requestId=\" + ReqID); }");
		sb.append("</script>");
		sb.append("</html>");

		return sb.toString();
	}

	@Override
	public void updateStatus(int authority, int recid) {
		Connection conn = cf.getConnection();
		String sql = "{ call updateStatus(?,?)";
		CallableStatement call;
		try {
			call = conn.prepareCall(sql);
			call.setInt(1, authority);
			call.setDouble(2, recid);
			call.executeQuery();

		} catch (SQLException e) {
			//
			e.printStackTrace();
		}
	}
	
	public void updateFinalGrade(String grade, int recid) {
		Connection conn = cf.getConnection();
		String sql = "{ call updateFinalGrade(?,?)";
		CallableStatement call;
		try {
			call = conn.prepareCall(sql);
			call.setString(1, grade);
			call.setDouble(2, recid);
			call.executeQuery();

		} catch (SQLException e) {
			//
			e.printStackTrace();
		}
	}

	@Override
	public void updateReimbursement(int id, double bal) {

		Connection conn = cf.getConnection();
		String sql = "{ call updateReimburse(?,?)";
		CallableStatement call;
		try {
			call = conn.prepareCall(sql);
			call.setInt(1, id);
			call.setDouble(2, bal);
			call.executeQuery();

		} catch (SQLException e) {
			//
			e.printStackTrace();
		}

	}

	public void updateFlagged(int requestId, int auth) {
		Connection conn = cf.getConnection();
		String sql = "{ call updateFlagged(?,?)";
		CallableStatement call;
		try {
			call = conn.prepareCall(sql);
			call.setInt(1, requestId);
			call.setDouble(2, auth);
			call.executeQuery();

		} catch (SQLException e) {
			//
			e.printStackTrace();
		}

	}

	public void updateInfo(String info, int recid) {
		Connection conn = cf.getConnection();
		String sql = "{ call updateInfo(?,?)";
		CallableStatement call;
		try {
			call = conn.prepareCall(sql);
			call.setString(1, info);
			call.setDouble(2, recid);
			call.execute();

		} catch (SQLException e) {
			//
			e.printStackTrace();
		}
	}

	public String getInfo(int reqid) {
		// TODO Auto-generated method stub
		Connection conn = cf.getConnection();
		String sql = "select moreinfo from request where requestid = ?";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ps.setInt(1, reqid);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				return rs.getString(1);
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return null;
	}
	
	public int getGradingScale(int reqid) {
		// TODO Auto-generated method stub
		Connection conn = cf.getConnection();
		String sql = "select gradingscale from request where requestid = ?";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ps.setInt(1, reqid);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				return rs.getInt(1);
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return -1;
	}

	@Override
	public double getPendingBalance(int userid) {
		// TODO Auto-generated method stub
		Connection conn = cf.getConnection();
		String sql = "select sum(reimbursment) from request where status!= -1 and status!= 3 and request.userid=?";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ps.setInt(1, userid);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				return rs.getDouble(1);
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return -1.0;
	}
}