...
public class RequestsDaoImipl implements RequestsDao {
	...
	private String basicMoreInfoTableToHtmlString(ArrayList<AppRequest> aar) {
		...
		for (AppRequest a : aar) {
			...
			"<input type=\"text\" class=\"submit\" placeholder = \"Add more info\" name=\"requestId\" value=\"");
			sb.append(a.getRequestid() + "\" onclick=\"deny(value)\"></form></td></tr>");
		}
		sb.append("</tbody></table></div></body>");
		sb.append(
				"<script>window.onload = function() {let table = document.getElementById(\"tbody\");console.log(\"in window onload function\");for (var i = 0, row; row = table.rows[i]; i++){let tableDate = row.cells[4].innerHTML;tableDate = new Date(tableDate);tableDate.setFullYear(Math.abs(tableDate.getFullYear()));let curDate = new Date();curDate.setHours(0,0,0,0);if (tableDate-curDate <= 86400000*14) {row.style.color=\"red\";}else{break;}}};");
